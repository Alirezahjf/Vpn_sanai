...
scripts/                دستورات مستقل: add-client, show-clients, backup, security, status, uninstall
config/defaults.conf    همهٔ پیش‌فرض‌ها (پورت‌ها، نامزدهای SNI، cron، …)
tests/                  تست‌های خودکار + پنل شبیه‌سازی‌شده (mock_panel.py)
```
...
پیش از حذف به‌طور خودکار یک پشتیبان `pre-uninstall` گرفته می‌شود. پورت SSH اضافه‌شده و قواعد فایروال نیز پاک‌سازی می‌شوند.
